...
**开发步骤**:
- [ ] 创建 `execute_psi` 函数
- [ ] 添加 `@TaskDispatcher.register_task('psi')` 装饰器
- [ ] 解析任务配置（keys、input_paths、output_paths、receiver等）
- [ ] 调用 `spu.psi()` 执行PSI
- [ ] 读取输出文件统计交集数量
- [ ] 返回标准化结果字典
- [ ] 添加完整的参数验证
...
**开发步骤**:
- [ ] 创建 `execute_standard_scaler` 函数
- [ ] 添加装饰器注册
- [ ] 解析配置（input_data、output_data、columns、with_mean、with_std）
- [ ] 使用 `sf.data.vertical.read_csv()` 读取VDataFrame
- [ ] 创建 `StandardScaler` 实例
- [ ] 调用 `fit_transform()` 执行标准化
- [ ] 保存结果到output_data指定路径
- [ ] 返回标准化参数（mean、std）
- [ ] 添加参数验证和错误处理
...
**开发步骤**:
- [ ] 创建 `execute_ss_logistic_regression` 函数
- [ ] 添加装饰器注册
- [ ] 解析配置（train_data、features、label、model_output、params）
- [ ] 读取训练数据为VDataFrame
- [ ] 创建 `SSRegression(spu)` 模型实例
- [ ] 调用 `model.fit()` 训练模型
- [ ] 保存模型到指定路径
- [ ] 返回训练结果和模型路径
- [ ] 添加训练参数验证
