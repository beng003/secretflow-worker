...
class PSIExecutor(BaseTaskExecutor):
    ...
    def validate_params(self) -> bool:
        ...
        try:
            ...
            valid_protocols = [
                'KKRT_PSI_2PC', 'ECDH_PSI_2PC', 'BC22_PSI_2PC',
                'ECDH_PSI_3PC'
            ]
            ...
            logger.info(f"PSI任务参数验证通过，参与方: {list(input_parties)}, 协议: {protocol}")
        ...
        try:
            ...
            if isinstance(psi_reports, list):
                for report in psi_reports:
                    if hasattr(report, 'intersection_count') and report.intersection_count >= 0:
            ...
            # 处理单个对象类型的报告
                ...
                elif isinstance(psi_reports, dict) and 'intersection_count' in psi_reports:
                    intersection_count = psi_reports['intersection_count']
