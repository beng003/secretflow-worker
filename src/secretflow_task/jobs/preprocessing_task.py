...
def _validate_standard_scaler_config(task_config: Dict) -> None:
    ...
    if input_parties != output_parties:
        raise ValueError(f"input_data和output_data的参与方不一致: {input_parties} vs {output_parties}")
...
def execute_standard_scaler(devices: Dict[str, PYU], task_config: Dict) -> Dict:
    ...
    Args:
        ...
        task_config: 任务配置字典，包含:
            - input_data: Dict[str, str] - 输入数据路径，键为参与方名称，值为CSV文件路径
            - output_data: Dict[str, str] - 输出数据路径，键为参与方名称，值为CSV文件路径
            - columns: Optional[List[str]] - 需要标准化的列名列表，None表示所有数值列
    ...
    try:
        ...
        logger.info(f"StandardScaler配置: parties={parties}, columns={columns}, with_mean={with_mean}, with_std={with_std}")
