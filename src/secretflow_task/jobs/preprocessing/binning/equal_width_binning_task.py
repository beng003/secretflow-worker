...
@TaskDispatcher.register_task("equal_width_binning")
def execute_equal_width_binning(devices: Dict[str, PYU], task_config: Dict) -> Dict:
    """
    执行EqualWidthBinning等宽分箱任务

    将连续型特征按等宽区间进行分箱，每个箱的宽度相等。
    ...
    Returns:
        Dict: 分箱结果
    """
    logger.info("开始执行EqualWidthBinning等宽分箱任务")
    ...
    try:
        ...
        logger.info(f"EqualWidthBinning配置: parties={parties}, columns={columns}, n_bins={n_bins}")
        ...
        # 等宽分箱
        def equal_width_binning(data, columns, n_bins):
            """在PYU上执行等宽分箱"""
            import pandas as pd
        ...
        for party in parties:
            ...
            if party_columns:
                vdf.partitions[pyu] = pyu(equal_width_binning)(
        ...
        logger.info("EqualWidthBinning任务执行成功")
        return result
