...
def _save_ss_lr_model_secure(
    ...
    # 保存主模型元数据文件
    meta_path = f"{model_output}.meta.json"
    with open(meta_path, 'w') as f:
        json.dump(model_meta, f, indent=2)
    logger.info(f"模型元数据已保存到: {meta_path}")
    
    # 为每个参与方保存密文分片
    # SPU.dump需要为每个参与方提供一个路径
    ...
    # 为每个参与方创建引用文件
    for party, share_path in zip(parties, share_paths):
        party_meta = {
            "party": party,
            "model_reference": meta_path,
            "share_path": share_path,
            "model_type": "ss_sgd_secure",
            "secure_mode": True,
            "created_at": datetime.now().isoformat()
        }
        party_meta_path = f"{model_output}.{party}.json"
        with open(party_meta_path, 'w') as f:
            json.dump(party_meta, f, indent=2)
        logger.info(f"参与方 {party} 的引用已保存到: {party_meta_path}")


def load_ss_lr_model_secure(model_output: str, spu_device: SPU, parties: List[str]) -> Dict:
    """
    安全加载SS-LR模型（从密文分片）
...
@TaskDispatcher.register_task('ss_lr_secure')
def execute_ss_logistic_regression_secure(devices: Dict[str, PYU], task_config: Dict) -> Dict:
    """
    ...
    try:
        ...
        # 验证配置（复用原有验证函数）
        from .ml_task import _validate_ss_lr_config
...
@TaskDispatcher.register_task('ss_lr_predict_secure')
def execute_ss_lr_predict_secure(devices: Dict[str, PYU], task_config: Dict) -> Dict:
    """
    执行SS-LR模型预测任务（安全模式）
    ...
    try:
        ...
        logger.info(f"在接收方 {receiver_party} 上保存预测结果到: {output_path}")
