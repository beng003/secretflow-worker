...
def _validate_psi_config(task_config: Dict[str, Any]) -> None:
    ...
    if parties != set(input_paths.keys()):
        raise ValueError(f"keys和input_paths的参与方不一致: {parties} vs {set(input_paths.keys())}")
    
    if parties != set(output_paths.keys()):
        raise ValueError(f"keys和output_paths的参与方不一致: {parties} vs {set(output_paths.keys())}")
    ...
    for party, key_list in keys.items():
        if not isinstance(key_list, list):
            raise ValueError(f"参与方 '{party}' 的keys必须是列表类型，当前类型: {type(key_list)}")
        if not key_list:
...
def execute_psi(devices: Dict[str, SPU | PYU], task_config: Dict[str, Any]) -> Dict[str, Any]:
    """
    执行PSI隐私集合求交任务
    ...
    try:
        ...
        logger.info(f"PSI配置: protocol={protocol}, receiver={receiver}, parties={list(keys.keys())}")
