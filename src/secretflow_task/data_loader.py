...
class DataLoader:
    ...
    def load_party_data(self, party_name: str, data_path: str) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(
                error_msg, extra={"party_name": party_name, "data_path": data_path}
            )
    ...
    def create_secretflow_data(
        self, data_config: Dict[str, str], devices: Dict[str, Any]
    ) -> Any:
        """创建secretflow数据对象
    ...
    def validate_data_schema(
        self, data: pd.DataFrame, required_columns: List[str]
    ) -> bool:
        """验证数据模式
...
class ResultManager:
    ...
    def save_results(
        self, results: Dict[str, Any], output_config: Dict[str, Any]
    ) -> List[str]:
        """保存计算结果
