...
class AlgorithmMetadata:
    ...
    def is_compatible(self, required_version: str = None) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"无法解析版本号: current={self.version}, required={required_version}"
            )
            return True
...
class AlgorithmRegistry:
    ...
    def register(
        ...
        try:
            ...
            if not inspect.isclass(executor_class):
                raise ValueError(
                    f"executor_class必须是类，而不是 {type(executor_class)}"
                )
    ...
    def list_algorithms(self, include_deprecated: bool = False) -> List[str]:
        ...
        if include_deprecated:
            ...
            return [
                name for name, meta in self._registry.items() if not meta.deprecated
            ]
    ...
    def check_compatibility(
        ...
        if not metadata:
            return {"compatible": False, "reason": f"算法 '{name}' 不存在"}
        ...
        if required_version and not metadata.is_compatible(required_version):
            issues.append(
                f"版本不兼容: 需要 >= {required_version}, 当前 {metadata.version}"
            )
        ...
        if num_parties is not None:
            if num_parties < metadata.min_parties:
                issues.append(
                    f"参与方数量不足: 最少需要 {metadata.min_parties}, 当前 {num_parties}"
                )
            if num_parties > metadata.max_parties:
                issues.append(
                    f"参与方数量超限: 最多支持 {metadata.max_parties}, 当前 {num_parties}"
                )
        ...
        return {"compatible": len(issues) == 0, "issues": issues, "metadata": metadata}
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_algorithms": len(
                [m for m in self._registry.values() if not m.deprecated]
            ),
            "deprecated_algorithms": len(
                [m for m in self._registry.values() if m.deprecated]
            ),
...
class AlgorithmFactory:
    ...
    def create_executor(
        ...
        try:
            ...
            if not self._registry.is_registered(task_type):
                ...
                raise ValueError(
                    f"不支持的任务类型: '{task_type}'。可用算法: {available}"
                )
    ...
    def _validate_compatibility(
        self, task_type: str, task_config: Dict
    ) -> Dict[str, Any]:
        """验证算法兼容性
        ...
        # 尝试从不同的配置字段提取参与方数量
        if "input_paths" in task_config and isinstance(
            task_config["input_paths"], dict
        ):
        ...
        return self._registry.check_compatibility(
            name=task_type, required_version=required_version, num_parties=num_parties
        )
...
def register_algorithm(
    name: str, executor_class: Type[BaseTaskExecutor], **kwargs
) -> bool:
    """注册算法的便捷函数
