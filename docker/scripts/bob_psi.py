...
def run_psi_participant():
    ...
    try:
        ...
        # 启动 Bob 的 Ray head 节点
        import ray
        logger.info("Starting Bob Ray head node...")
        ray.init(
            include_dashboard=False,
            ignore_reinit_error=True
        )
        logger.info("Bob Ray head node started successfully")
        ...
        # Bob 连接到自己的 Ray head 地址（生产模式）
        bob_ray_address = 'localhost:10001'  # Ray默认端口
        
        logger.info(f"Initializing SecretFlow with Bob Ray address: {bob_ray_address}")
        logger.info(f"Cluster config: {cluster_config}")
        
        # 根据生产模式文档初始化 SecretFlow
        sf.init(address=bob_ray_address, cluster_config=cluster_config)
