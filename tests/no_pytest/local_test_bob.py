...
def local_psi_test() -> Dict[str, Any]:
    ...
    sf.init(address = "127.0.0.1:7801", cluster_config=cluster_config, ray_mode = False)
    ...
    # 1. 单键隐私求交
    input_path = {
        alice.party: f'{test_data_path}/input.csv', 
        bob.party: f'{test_data_path}/bob.csv'
    }
    output_path = {
        alice.party: f'{test_data_path}/output.csv', 
        bob.party: f'{test_data_path}/bob_psi.csv'
    }
    sf.wait(spu.psi(
        {alice.party:["id"], bob.party:["uid"]}, 
