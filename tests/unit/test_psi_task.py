...
def setup_secretflow():
    ...
    # 创建测试数据文件
    da, db = data.sample(frac=0.9, random_state=42), data.sample(frac=0.8, random_state=42)
...
class TestPSIConfigValidation:
    ...
    def test_validate_psi_config_success(self):
        ...
        config = {
            ...
            "input_paths": {"alice": f"{TEST_DATA_DIR}/alice.csv", "bob": f"{TEST_DATA_DIR}/bob.csv"},
            "output_paths": {"alice": f"{TEST_DATA_DIR}/alice_psi.csv", "bob": f"{TEST_DATA_DIR}/bob_psi.csv"},
            "receiver": "alice"
        }
...
class TestExecutePSI:
    ...
    def test_execute_psi_success(self, real_devices):
        ...
        task_config = {
            "keys": {"alice": ["uid"], "bob": ["uid"]},
            "input_paths": {"alice": f"{TEST_DATA_DIR}/alice.csv", "bob": f"{TEST_DATA_DIR}/bob.csv"},
            "output_paths": {"alice": f"{TEST_DATA_DIR}/alice_psi_test1.csv", "bob": f"{TEST_DATA_DIR}/bob_psi_test1.csv"},
            "receiver": "alice",
    ...
    def test_execute_psi_multi_column(self, real_devices):
        ...
        task_config = {
            "keys": {"alice": ["uid", "month"], "bob": ["uid", "month"]},
            "input_paths": {"alice": f"{TEST_DATA_DIR}/alice.csv", "bob": f"{TEST_DATA_DIR}/bob.csv"},
            "output_paths": {"alice": f"{TEST_DATA_DIR}/alice_psi_test2.csv", "bob": f"{TEST_DATA_DIR}/bob_psi_test2.csv"},
            "receiver": "alice"
        }
...
class TestPSITaskIntegration:
    ...
    def test_psi_task_via_dispatcher(self, real_devices):
        ...
        task_config = {
            "keys": {"alice": ["uid"], "bob": ["uid"]},
            "input_paths": {"alice": f"{TEST_DATA_DIR}/alice.csv", "bob": f"{TEST_DATA_DIR}/bob.csv"},
            "output_paths": {"alice": f"{TEST_DATA_DIR}/alice_psi_test3.csv", "bob": f"{TEST_DATA_DIR}/bob_psi_test3.csv"},
            "receiver": "alice"
        }
