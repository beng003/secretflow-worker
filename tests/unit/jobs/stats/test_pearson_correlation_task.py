...
def prepare_test_data():
    ...
    alice_path = os.path.join(TEST_DATA_DIR, "alice_corr.csv")
    bob_path = os.path.join(TEST_DATA_DIR, "bob_corr.csv")
...
def test_pearson_correlation(setup_devices, prepare_test_data):
    """测试Pearson相关性分析任务"""
    ...
    task_config = {
        ...
        "output_report": os.path.join(TEST_DATA_DIR, "pearson_corr_report.json"),
    }
    ...
    for col in task_config["columns"]:
        assert col in corr_matrix
        assert len(corr_matrix[col]) == 4
    
    print(f"\nPearson相关性分析结果: {result}")
