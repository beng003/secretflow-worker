...
class TestStandardScalerConfigValidation:
    ...
    def test_validate_config_success(self):
        """测试有效的配置"""
        config = {
            "input_data": {"alice": f"{TEST_DATA_DIR}/alice_raw.csv", "bob": f"{TEST_DATA_DIR}/bob_raw.csv"},
            "output_data": {"alice": f"{TEST_DATA_DIR}/alice_scaled.csv", "bob": f"{TEST_DATA_DIR}/bob_scaled.csv"},
            "columns": ["age", "income"],
            "with_mean": True,
            "with_std": True
        }
...
class TestExecuteStandardScaler:
    ...
    def test_execute_standard_scaler_file_not_exist(self, real_devices):
        """测试输入文件不存在"""
        task_config = {
            "input_data": {"alice": f"{TEST_DATA_DIR}/nonexistent.csv", "bob": f"{TEST_DATA_DIR}/bob_raw.csv"},
            "output_data": {"alice": f"{TEST_DATA_DIR}/alice_scaled.csv", "bob": f"{TEST_DATA_DIR}/bob_scaled.csv"},
        }
    ...
    def test_execute_standard_scaler_all_columns(self, real_devices):
        ...
        task_config = {
            "input_data": {"alice": f"{TEST_DATA_DIR}/alice_raw.csv", "bob": f"{TEST_DATA_DIR}/bob_raw.csv"},
            "output_data": {"alice": f"{TEST_DATA_DIR}/alice_scaled_all.csv", "bob": f"{TEST_DATA_DIR}/bob_scaled_all.csv"},
            "with_mean": True,
            "with_std": True
        }
    ...
    def test_execute_standard_scaler_specific_columns(self, real_devices):
        ...
        task_config = {
            "input_data": {"alice": f"{TEST_DATA_DIR}/alice_raw.csv", "bob": f"{TEST_DATA_DIR}/bob_raw.csv"},
            "output_data": {"alice": f"{TEST_DATA_DIR}/alice_scaled_specific.csv", "bob": f"{TEST_DATA_DIR}/bob_scaled_specific.csv"},
            "columns": ["age", "income"],
            "with_mean": True,
            "with_std": True
        }
    ...
    def test_execute_standard_scaler_without_mean(self, real_devices):
        ...
        task_config = {
            "input_data": {"alice": f"{TEST_DATA_DIR}/alice_raw.csv", "bob": f"{TEST_DATA_DIR}/bob_raw.csv"},
            "output_data": {"alice": f"{TEST_DATA_DIR}/alice_scaled_nomean.csv", "bob": f"{TEST_DATA_DIR}/bob_scaled_nomean.csv"},
            "with_mean": False,
            "with_std": True
        }
...
class TestStandardScalerIntegration:
    ...
    def test_standard_scaler_via_dispatcher(self, real_devices):
        ...
        task_config = {
            "input_data": {"alice": f"{TEST_DATA_DIR}/alice_raw.csv", "bob": f"{TEST_DATA_DIR}/bob_raw.csv"},
            "output_data": {"alice": f"{TEST_DATA_DIR}/alice_scaled_dispatcher.csv", "bob": f"{TEST_DATA_DIR}/bob_scaled_dispatcher.csv"},
            "columns": ["age", "income"],
            "with_mean": True,
            "with_std": True
        }
